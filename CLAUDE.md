...
- New homepage sections follow the Services card pattern (`src/components/services/index.tsx`): `rounded-xl border border-border p-4 space-y-2 bg-muted/20`.
- `/library` sub-sections use a **compact log-row pattern** (see `src/components/library/currentReads.tsx` + `fragranceRotation.tsx`): a bordered section header with a right-rail entry count (e.g. `04 entries`), followed by a `divide-y divide-border <ul>` of rows. Each row has a colored status dot + label above the title, a right-aligned meta field on the same line as the title, and a one-line `font-mono` description below. Designed to stay scannable as content grows — avoid the card-grid pattern here.
- Featured/highlight items inside a section (e.g. the Signature Blend fragrance) get a dashed-border card on top of a diagonal-line backdrop with a corner tag — reserve this treatment for one-of-a-kind items, not regular entries.
...
- Route content is wrapped in `<main>` inside `src/app/layout.tsx` (not in individual pages) — don't add a second `<main>` in page components.
